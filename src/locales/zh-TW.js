import DFProject from "../views/DFProject.vue";
...
export default {
  ...
  about: {
    intro: `我是一名 遊戲開發者，我會C#程式開發、3D模型製作、場景設計美術，以及各種美術工具與。
    擅長開放世界環境、Shader、程序化流程、風格化或寫實、渲染優化、PBR 工作流程等。`,
  ...
  code: {
    ...
    tool2: {
      title: "迷宮生成工具",
      description:
        "在 Editor 中載入模組化物件，一鍵生成迷宮關卡，生成後美術也可以手動刪除與調整。",
      features: [
    ...
    tool3: {
      title: "迷宮裝飾工具",
      description:
        "判斷迷宮牆邊並載入裝飾物件，透過調整數值自動生成樹木、石頭等物件。",
      features: [
    ...
    tool4: {
      title: "NavMesh 生成小地圖",
      description:
        "從 NavMesh 自動生成遊戲小地圖 PNG，可自訂區塊顏色並直接用在 UI 上。",
      features: [
  ...
  eecbet: {
    ...
    myWorkItems: [
      ...
      {
        title: "主頁設計與開發",
        desc: "首頁設計，包含輪播橫幅、推薦遊戲等功能模組",
      },
      {
        title: "老虎機頁面",
        desc: "頁面設計，串接資料庫連動網頁更新，輪播遊戲 UI 功能",
      },
  ...
  DFProject: {
    ...
    tabLabels: ["端午活動", "億萬活動", "個人訊息", "勳章和抽獎"],

    myWorkTitle: "流程&負責細項",
    archRootLabel: "後端伺服器",
    archChildren: [
      {
        label: "Manager 後端資料傳送和接收",
        sub: "協議收發 / 資料邏輯 ",
        children: [
          {
            label: "Control 美術畫面串接",
            sub: "動畫串接 / 任務互動 / 排行榜預覽 ",
          },
          {
            label: "RankingList 子畫面串接",
            sub: "完整排行榜 / 今昨日切換 / 定時自動更新 ",
          },
        ],
      },
    ],
        ...
        children: [
          {
            ...
            children: [
              {
                label: "Control 二級畫面串接",
                sub: "列表對應資料 /解鎖排列 / 進度顯示",
                children: [
                  {
                    label: "Control 三級畫面串接",
                    sub: "拖曳配戴 /本地暫存 / 確認顯示發送",
                  },
                ],
              },
              {
    ...
    techStacks: [
      {
        title: "動畫與非同步",
        desc: "等動畫結束才觸發後續流程，讓動畫節奏跟網路延遲解耦",
      },
      {
        title: "排行榜排程和刷新",
        desc: "使用 Timer 維持更新，並在後端限流時從快取還原排名",
      },
      {
        title: "資源需求設計",
        desc: "道具需求量用表格定義，數值調整只需改表，不需改程式",
      },
    ],
    techStacks2: [
      {
        title: "刷新與對齊",
        desc: "用時間與金額結算同步刷新，並加上緩衝時間避免抓到舊資料",
      },
      {
        title: "倒數計時製作",
        desc: "使用版本號控制比對，避免頁面關閉後殘留的流程干擾下一輪動畫",
      },
      {
        title: "資料呈現處理",
        desc: "處理初始金額基準、未上榜時的差距提示，避免畫面異常",
      },
    ],
    techStacks3: [
      {
        title: "換頭像流程",
        desc: "等後端確認成功才寫回全域資料並刷新畫面，失敗則維持原狀，避免請求競速與資料不一致",
      },
      {
        title: "物件池管理",
        desc: "用模板物件生成實例，依資料長度決定顯示數量，已存在物件就隱藏，避免重複生成銷毀",
      },
    ],
    techStacks4: [
      {
        title: "拖曳配戴系統設計",
        desc: "拖曳互動，以及拿掉後自動往前補位",
      },
      {
        title: "本地暫存",
        desc: "拖曳過程只更新本地變數，按下確認才一次送出給後端，避免頻繁請求造成延遲與負擔",
      },
      {
        title: "解鎖狀態雙層排序",
        desc: "先抓解鎖狀態、狀態相同再比權重，讓已解鎖項目優先顯示",
      },
      {
        title: "動畫計時器防護",
        desc: "觸發新動畫前先清除舊計時器；防護機制依情境複雜度取捨，不是每個非同步流程都需要最複雜的解法",
      },
    ],
  },
};
